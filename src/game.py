...
class App:
    ...
    def load_images(self):
        # We put all the sprites (which are different color blocks) in a list
        # We don't need to resize the sprites, we created them with the correct size
        images = []
        for i in range(7):
    ...
    def run(self):
        while True:
            if self.tetris.game_over_flag: # We detect if the game is over
                Endgame(self.screen).run() # The page changes to the Endgame page
                self.tetris.game_over_flag = False
            else: # While the game runs :
                self.check_events()
                self.update()
                self.draw()
...
class Username:
    def __init__(self, screen):
        ...
        self.screen = screen
        self.list_username = ""
        self.under_username = "" # This text will change when the user enter his name
    ...
    def draw(self):
        ...
        # before_username will be replaced by list_username when the player enter a character
...
class Endgame:
    ...
    def save_username(self, username):
        ...
        except ValueError: # If the json file is empty, to avoid error, we create the main content
            data = {
                "Current score": "",
                "Users and Scores": {}
            }

        if username not in [i for i in data["Users and Scores"]]: # If there is a new player
            data["Users and Scores"][username] = data["Current score"]

        # If the player is known, the score will be save only if it is higher than the saved score
        elif data["Users and Scores"][username] < data["Current score"]: 
            data["Users and Scores"][username] = data["Current score"]

        data["Current score"] = 0 # The current score is then changed to 0

        with open("data/score.json", "w") as file:
            json.dump(data, file, indent=4) # Indent is used to write correctly in the json file
