...
class Block(pg.sprite.Sprite):
    ...
    def is_collide(self, pos):
        ...
        if 0 <= x <= FIELD_W - 1 and y < FIELD_H and (
...
class Tetromino:
    ...
    def rotate(self):
        ...
        new_blocks_positions = [block.rotate(pivot_pos) for block in self.blocks]

        if not self.is_collide(new_blocks_positions):
            for i, block in enumerate(self.blocks):
                block.pos = new_blocks_positions[i]
